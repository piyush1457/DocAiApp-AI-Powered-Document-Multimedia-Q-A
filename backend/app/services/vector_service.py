...
class FAISSVectorStore:
    ...
    async def ensure_index_loaded(self, user_id: str, file_id: str, db):
        ...
        if not os.path.exists(index_file):
            logger.warning(f"FAISS index not found for user {user_id}, rebuilding from DB")
            ...
            if chunks:
                ...
                logger.info(f"Rebuilt index for user {user_id} with {len(chunks)} chunks")
            else:
                logger.warning(f"No chunks found in DB for file {file_id}, cannot build index")
    ...
    async def similarity_search(self, query: str, user_id: str, file_id: str, top_k: int = 6) -> List[ChunkResult]:
        """
    ...
    def delete_by_file_id(self, user_id: str, file_id: str) -> None:
        ...
        ids_to_remove = [
            int(idx) for idx, meta in metadata.items() 
