...
async def test_chat_sse_stream_success(client, auth_headers, db_session, test_user):
    ...
    with patch("app.services.vector_service.vector_service.search", AsyncMock(return_value=[])), \
         patch("app.services.llm_service.llm_service.get_chat_response_stream") as mock_stream:
        
        async def mock_gen(*args, **kwargs):
            yield "data: " + json.dumps({"token": "Hello"}) + "\n\n"
            yield "data: " + json.dumps({"metadata": {"sources": [{"page": 1}]}}) + "\n\n"
        ...
        async with client.stream("POST", "/docaiapp/v1/chat/", json=payload, headers=auth_headers) as response:
