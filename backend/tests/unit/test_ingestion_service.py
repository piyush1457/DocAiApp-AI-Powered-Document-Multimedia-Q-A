...
async def test_process_file_pdf_success(db_session, test_file):
    ...
    with (
        patch("app.services.ingestion_service.parse_pdf") as mock_parse,
        patch(
            "app.services.ingestion_service.FAISSVectorStore.add_chunks",
            return_value=[123],
        ),
    ):
...
async def test_process_file_audio_success(db_session, test_user):
    ...
    with (
        patch(
            "app.services.ingestion_service.TranscriptionService.transcribe_file"
        ) as mock_transcribe,
        patch(
            "app.services.ingestion_service.FAISSVectorStore.add_chunks",
            return_value=[456],
