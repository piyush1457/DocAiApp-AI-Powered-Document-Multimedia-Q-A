...
def test_split_audio_success(transcription_service):
    ...
    with patch("subprocess.check_output") as mock_probe, \
         patch("subprocess.run") as mock_run:
        mock_probe.return_value = b"1200\n" # 20 minutes
        mock_run.return_value = MagicMock(returncode=0)
        
        chunks = transcription_service.split_audio("test.mp3", chunk_duration_sec=600, overlap_sec=60)
...
async def test_transcribe_file_offset_correction(transcription_service):
    ...
    with patch("os.path.getsize", return_value=30 * 1024 * 1024), \
         patch.object(transcription_service, "split_audio", return_value=["c1.mp3", "c2.mp3"]), \
        ...
        mock_whisper.side_effect = [
            ...
            [{"text": "t2", "start": 0, "end": 5}]  # Chunk 2 (offset 540 if overlap=60)
        ]
...
async def test_transcribe_persistence(transcription_service):
    ...
    with patch("os.path.getsize", return_value=1 * 1024 * 1024), \
         patch.object(transcription_service, "_call_whisper", return_value=[{"text": "persist me", "start": 0, "end": 2}]), \
         patch("os.remove"):
...
def test_parse_segments_from_json(transcription_service, verbose_json_response):
    ...
    with patch("app.services.transcription_service.client.audio.transcriptions.create") as mock_whisper:
