...
from app.core.dependencies import get_current_user, RateLimiter, AuthRateLimiter
...
async def test_get_current_user_not_found(db_session):
    with patch("app.core.dependencies.decode_token", return_value={"sub": "123"}), \
...
async def test_auth_rate_limiter_exceeded():
    ...
    with patch.object(limiter.redis, "pipeline") as mock_pipe:
        mock_pipe.return_value.__aenter__.return_value.execute = AsyncMock(return_value=[None, 2])
        with pytest.raises(HTTPException) as exc:
