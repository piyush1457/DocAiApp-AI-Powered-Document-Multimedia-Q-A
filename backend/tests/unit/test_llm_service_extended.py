...
async def test_map_reduce_summary(llm_service):
    ...
    mock_response.choices = [MagicMock(message=MagicMock(content='{"summary": "s", "key_topics": [], "word_count": 1}'))]
    
    with patch.object(llm_service, "_single_pass_summary", AsyncMock(return_value={"summary": "s", "key_topics": [], "word_count": 1})) as mock_single:
...
async def test_generate_embeddings_cache_hit(llm_service):
    ...
    with patch.object(cache_service, "get", AsyncMock(return_value=[0.1, 0.2])), \
