...
async def test_low_context_fallback(llm_service):
    ...
    async def mock_generator(*args, **kwargs):
        ...
        mock_chunk.choices = [
            MagicMock(delta=MagicMock(content="I don't have enough context."))
        ]
        yield mock_chunk
    ...
    with patch.object(
        ...
        "create",
        AsyncMock(side_effect=mock_generator),
    ):
...
async def test_llm_error_wrapping(llm_service):
    ...
    with patch.object(
        llm_service.client.embeddings, "create", side_effect=Exception("API Down")
    ):
