...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE filetype AS ENUM ('PDF', 'MP3', 'MP4', 'WAV', 'M4A', 'WEBM')"
    )
    op.execute(
        "ALTER TABLE files ALTER COLUMN file_type TYPE filetype USING file_type::text::filetype"
    )
    ...
    op.execute(
        "ALTER TABLE files ALTER COLUMN status TYPE filestatus USING status::text::filestatus"
    )
